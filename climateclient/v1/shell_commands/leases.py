...
class ListLeases(command.ListCommand):
    ...
    list_columns = ['id', 'name', 'start_date', 'end_date', 'status']
...
class CreateLease(command.CreateCommand):
    """Only services may create lease, not real user.

    It happens because we need id of the reserved resource to create lease.
    When service creates reserved resource (Nova-VM, Cinder-volume, etc.) it
    comes to Climate and creates lease via Python client.
    """
    pass
